...
source("/home/redgar/Documents/EBI/scRNAseq_codon/scripts/00_pretty_plots.R")

load("/media/redgar/Seagate Portable Drive1/EBI_backup/desktop_august2022/Documents/codon/scRNAseq_codon/data/d10x.primary_epi_raw.RData")

MHCI = c('HLA-F', 'HLA-G', 'HLA-A', 'HLA-E', 'HLA-C', 'HLA-B',"TAP1","TAP2","PSMB9","PSMB8","B2M","IRF1","NLRC5")
d10x.primary_epi_raw
## data already normalized but still raw with no integration




## testing factor
Idents(d10x.primary_epi_raw) <- "orig.ident"
table(d10x.primary_epi_raw$orig.ident)
...
contrasts_diagnosis<-combinations(n = 3, r = 2, v = unique(d10x.primary_epi_raw$orig.ident), repeats.allowed = FALSE)


diff_exp_all<-lapply(1:nrow(contrasts_diagnosis), function(x){
  de<-FindMarkers(d10x.primary_epi_raw, ident.1 = contrasts_diagnosis[x,1], ident.2 = contrasts_diagnosis[x,2], test.use = "MAST",latent.vars="nFeature_RNA", verbose=F)
  print(paste(contrasts_diagnosis[x,1],"vs", contrasts_diagnosis[x,2],":", nrow(de), sep=" "))
  de$gene<-rownames(de)
  rownames(de)<-NULL
  de<-de[,c(6,1:5)]
  de$cell.1<-contrasts_diagnosis[x,1]
  de$cell.2<-contrasts_diagnosis[x,2]
  de})


diff_exp_all<-do.call(rbind, diff_exp_all)

save(diff_exp_all, file="/home/redgar/Documents/EBI/scRNAseq_codon/data/primary_epithelial_pseudobulk_diff_genes.RData"))


### test only MHCI (cause slow) but with not cutoffs for FC or pct 
d10x.primary_epi_raw_MHCI<-subset(d10x.primary_epi_raw, features = MHCI)

diff_exp_MHCI<-lapply(1:nrow(contrasts_diagnosis), function(x){
  de<-FindMarkers(d10x.primary_epi_raw_MHCI, ident.1 = contrasts_diagnosis[x,1], ident.2 = contrasts_diagnosis[x,2], test.use = "MAST",latent.vars="nFeature_RNA", verbose=F, logfc.threshold = 0, min.pct = 0)
  print(paste(contrasts_diagnosis[x,1],"vs", contrasts_diagnosis[x,2],":", nrow(de), sep=" "))
  de$gene<-rownames(de)
  rownames(de)<-NULL
  de<-de[,c(6,1:5)]
  ...
  de$cell.2<-contrasts_diagnosis[x,2]
  de})
diff_exp_MHCI<-do.call(rbind, diff_exp_MHCI)


## was NLRC5 even tested (or two lowly expressed?)
diff_exp_MHCI[grep("NLRC5",diff_exp_MHCI$gene),]

# add a significant threshold here for adjusted as not all in these lists are
diff_exp_MHCI_sig<-diff_exp_MHCI[which(diff_exp_MHCI$p_val_adj<0.005),]
...
## 
table(d10x.primary_epi_raw$orig.ident)
# NLRC5 is expressed in 106 CD epi cells, 114 Control cells, and 17 UC cells
...
cell_label<-cell_label[which(rownames(cell_label)%in%rownames(d10x.primary.cd.ctrl@meta.data)),]
identical(rownames(cell_label),rownames(d10x.primary.cd.ctrl@meta.data))
d10x.primary.cd.ctrl <- AddMetaData(d10x.primary.cd.ctrl, metadata = cell_label)

d10x.primary.cd.ctrl@meta.data$general_type<-"Immune"
d10x.primary.cd.ctrl@meta.data$general_type[which(d10x.primary.cd.ctrl@meta.data$seurat_clusters%in%c(12,14,25))]<-"Epithelial"
d10x.primary.cd.ctrl@meta.data$general_type[which(d10x.primary.cd.ctrl@meta.data$seurat_clusters%in%c(5,10,13,15,16,22,23,28,24))]<-"Stromal"

## testing factor
d10x.primary.cd.ctrl$generaltype_diagnosis<-paste(d10x.primary.cd.ctrl$general_type, d10x.primary.cd.ctrl$orig.ident, sep = "_")
Idents(d10x.primary.cd.ctrl) <- "generaltype_diagnosis"
...
general_types<-unique(d10x.primary.cd.ctrl$general_type)

contrasts_celltype_stim<-do.call(rbind,lapply(1:length(general_types), function(x){
  combinations(n = 2, r = 2, v = d10x.primary.cd.ctrl$generaltype_diagnosis[grep(general_types[x],d10x.primary.cd.ctrl$general_type)], repeats.allowed = FALSE)}))

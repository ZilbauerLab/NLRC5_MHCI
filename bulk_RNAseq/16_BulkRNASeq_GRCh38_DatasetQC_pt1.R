...
#     - AF raw counts: /home/fp215/rds/rds-fp215-working/RNASEQ/NLRC5_PAPER/RAW_COUNTS/AF_GRCh38_per-gene-counts.2.txt
...
raw.af <- read.table("/home/fp215/rds/rds-fp215-working/RNASEQ/NLRC5_PAPER/RAW_COUNTS/AF_GRCh38_per-gene-counts.2.txt", header=T, sep="\t", stringsAsFactors=F)
...
readme.af.raw <- data.frame(Dataset=c("raw.af","pheno.af","info"),Description=c("NGS-K.Nayak-40750 (AF) raw counts (n=90)","Corresponding sample info, updated 24Oct22","Ensembl GRCh38 release 99 gene annotation info"))
save(raw.af,pheno.af,info,readme.af.raw,file="RAW_COUNTS/AF_GRCh38_RawCounts.RData")
...
af.noiseq <- readData(data=raw.af, length=length, gc=gc, biotype=biotype, chromosome=coords, factors=pheno.af)
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_BiotypeDistribution.pdf", width=10, height=5)
explo.plot(biodetection, samples="TI", plottype="persample", toplot="protein_coding")
explo.plot(biodetection, samples="SC", plottype="persample", toplot="protein_coding")
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_CountDistribution.pdf", width=10, height=6)
explo.plot(counts, samples="TI", toplot="global", plottype="boxplot")
explo.plot(counts, samples="SC", toplot="global", plottype="boxplot")
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_ProteinCoding_CountDistribution.pdf", width=30, height=5)
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_ProteinCoding_PercentLowCount.pdf", width=30, height=5)
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_ProteinCoding_LengthBias.pdf", width=7, height=5)
explo.plot(len, samples=NULL, toplot="protein_coding")
dev.off()
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_ProteinCoding_GCBias.pdf", width=7, height=5)
explo.plot(raw.gc, samples=NULL, toplot="protein_coding")
dev.off()
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_RawEuclideanDistanceHeatmap.pdf", width=20, height=20)
matrix <- as.matrix(raw.dist)
...
a.dds <- DESeq(af.dds)
save(af.dds,af.vst,a.dds,pheno.af,file="RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_RawDataQCInput.RData")

pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_CooksCutoff_RawNoBatchCorrection.pdf", width=30, height=7)
...
save(af,pheno.af,info,readme.af.norm,file="NORMALISED_COUNTS/AF_GRCh38_NormalisedCounts.RData")

pca.raw <- plotPCA(af.vst, intgroup=c("group2"), returnData=T)
percentvar <- round(100*attr(pca.raw, "percentVar"))

pdf("RAW_COUNTS_QC/AF_GRCh37/AF_GRCh38_Raw_Tissue-Diagnosis-Treatment_PC1vPC2.pdf", width=10, height=10)
ggplot(pca.raw, aes(PC1, PC2, color=group2))+geom_point(size=3)+xlab(paste0("PC1: ",percentvar[1],"% variance"))+ylab(paste0("PC2: ",percentvar[2],"%variance"))
dev.off()
...
pdf("NORMALISED_COUNTS_QC/AF_GRCh38/AF_GRCh38_ALL_NormalisedCounts_PC1vPC2.pdf", width=10, height=10)
...
pdf("RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_SVACorrelations_5SVs.pdf", width=8, height=7)
a.cor$plot.input
dev.off()

write.table(a.cor$sv.correlation, "RAW_COUNTS_QC/AF_GRCh38/AF_GRCh38_SVACorrelations_5SVs.txt", row.names=F, sep="\t", quote=F)
